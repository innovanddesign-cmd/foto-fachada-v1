...
export function RemoteSyncPanel() {
 ...
 return <section className="studio-panel space-y-5" aria-busy={api.syncing || api.loading}>
   ...
   {!api.user ? <p className="studio-notice">Para guardar en tu cuenta y publicar, <a className="underline font-semibold" href="/auth/login">inicia sesión</a> o <a className="underline font-semibold" href="/auth/signup">crea una cuenta</a>. Tus borradores locales se conservan.</p> : <>
     ...
     {!api.loading && !api.error && api.remoteCampaigns.length === 0 && <div className="rounded-xl bg-slate-50 p-5"><h3 className="font-semibold">Tu cuenta todavía no tiene escaparates guardados</h3><p className="studio-muted mt-1">{current ? 'Guarda el borrador actual con el botón superior. Después aparecerá la opción Publicar.' : 'Crea un escaparate o guarda uno de tus borradores locales.'}</p></div>}
     <div className="space-y-3">{api.remoteCampaigns.map(c => <article key={c.id} className="border border-slate-200 rounded-2xl p-5">
       <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold text-lg break-words">{c.name}</h3><p className="studio-muted text-sm mt-1">{c.plan} · Guardado {new Date(c.updated_at).toLocaleDateString('es-ES')}</p></div><span className={c.status === 'active' ? 'text-emerald-800 bg-emerald-50 px-3 py-1 rounded-full text-sm' : 'text-slate-700 bg-slate-100 px-3 py-1 rounded-full text-sm'}>{c.status === 'active' ? 'Web publicada' : 'Borrador privado'}</span></div>
   ...
   {(failure || api.error) && <p role="alert" className="text-red-700 bg-red-50 p-4 rounded-xl">{failure || api.error}</p>}
   {message && <p role="status" className="text-emerald-800 bg-emerald-50 p-4 rounded-xl">{message}</p>}
 </section>;
}
