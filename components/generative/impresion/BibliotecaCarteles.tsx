 ...
 return <section className="grid lg:grid-cols-[minmax(260px,0.8fr)_minmax(0,1.2fr)] gap-6 items-start">
   <div className="space-y-6">
     <div className="studio-panel space-y-5">
       ...
       {busy && <p role="status">Preparando archivo…</p>}{message && <p role="status" className="text-emerald-800">{message}</p>}{error && <p role="alert" className="text-red-700">{error}</p>}
     </div>
     <div className="studio-panel"><h2 className="font-semibold mb-4">Diseños en este dispositivo ({s.cartelesGenerados.length})</h2>
       ...
       <ul className="space-y-3">{s.cartelesGenerados.map(c => <li key={c.id} className="rounded-xl border border-slate-200 p-3"><div className="flex flex-wrap items-center gap-2"><button className="studio-button flex-1" onClick={() => { setSelected(c.id); setFormat(c.formato); setHeadline(c.configVisual.fraseImpacto); setMessage('Diseño abierto.'); }}>{c.campana.nombre} · {c.formato}</button><button className="studio-button" aria-label={`Eliminar ${c.campana.nombre}`} onClick={() => setPendingDelete(c.id)}><Trash2 size={18} /></button></div>{pendingDelete === c.id && <div className="mt-3 space-y-2"><p>¿Eliminar este diseño local?</p><div className="flex gap-2"><button className="studio-button" onClick={() => { s.eliminarCartel(c.id); if (selected === c.id) setSelected(null); setPendingDelete(null); }}>Eliminar diseño</button><button className="studio-button" onClick={() => setPendingDelete(null)}>Cancelar</button></div></div>}</li>)}</ul>
     </div>
   </div>
   <div className="rounded-3xl bg-slate-200 p-4 sm:p-8 min-w-0"><p className="text-slate-600 text-sm mb-5 text-center">Vista previa · {format === 'SQUARE' ? 'Cuadrado' : format}</p><PosterViewport formato={format}><GeneradorCartel formato={format} configVisual={config} /></PosterViewport></div>
 </section>;
}
